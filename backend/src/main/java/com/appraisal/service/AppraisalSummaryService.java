...
public class AppraisalSummaryService {
    ...
    public AppraisalSummary calculateAndSaveSummary(Long employeeId, Long cycleId) {
        ...
        List<SelfAssessment> selfAssessments = selfAssessmentRepository.findByEmployeeIdAndCycleId(employeeId, cycleId);
        ...
        for (ManagerAssessment assessment : managerAssessments) {
            ...
            totalManagerWeight += weight;
        }

        double totalSelfScore = 0;
        double totalSelfWeight = 0;
        for (SelfAssessment assessment : selfAssessments) {
            double weight = assessment.getKpi().getWeightage();
            totalSelfScore += assessment.getSelfRating() * weight;
            totalSelfWeight += weight;
        }

        double avgManagerScore = totalManagerWeight > 0 ? totalManagerScore / totalManagerWeight : 0;
        double avgSelfScore = totalSelfWeight > 0 ? totalSelfScore / totalSelfWeight : 0;
        ...
        if (existingSummary.isPresent()) {
            ...
            summary.setAvgSelfScore(avgSelfScore);
